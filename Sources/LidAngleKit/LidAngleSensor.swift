...
public final class LidAngleSensor {
    ...
    public func angle() -> Double? {
        ...
        return Self.normalizedAngle(degrees)
    }

    /// A tiny negative calibration offset at the closed stop wraps to 359°.
    /// A clamshell uses the smaller circular angle, never a full rotation.
    static func normalizedAngle(_ degrees: Double) -> Double {
        min(degrees, 360 - degrees)
    }
